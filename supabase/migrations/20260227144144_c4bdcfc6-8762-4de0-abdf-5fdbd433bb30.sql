...
-- The issue is all policies were created as RESTRICTIVE (using CREATE POLICY ... AS RESTRICTIVE)
-- PostgreSQL requires at least one PERMISSIVE policy to grant access; RESTRICTIVE only narrows it
...
DROP POLICY IF EXISTS "Role view posts" ON public.posts;
...
-- ============ BRANCHES ============
DROP POLICY IF EXISTS "Role view branches" ON public.branches;
...
-- Admin also needs to read all branches (including inactive)
...
CREATE POLICY "Public read results of published exams" ON public.results FOR SELECT USING (
  (EXISTS (SELECT 1 FROM exams WHERE exams.id = results.exam_id AND exams.is_published = true)) OR is_admin()
);
...
-- Admin needs to see all books including inactive
...
DROP POLICY IF EXISTS "Public read own contacts by phone" ON public.contact_submissions;
...
CREATE POLICY "Users read own role permissions" ON public.admin_permissions FOR SELECT USING (
  role_name IN (SELECT role::text FROM user_roles WHERE user_id = auth.uid())
);
...
DROP POLICY IF EXISTS "Role view order items" ON public.book_order_items;
CREATE POLICY "Order owner or admin read items" ON public.book_order_items FOR SELECT USING (
  EXISTS (SELECT 1 FROM book_orders bo WHERE bo.id = book_order_items.order_id AND (bo.user_id = auth.uid() OR is_admin()))
);
