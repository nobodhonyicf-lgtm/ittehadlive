...
const AdminPolls = () => {
  ...
  const handleCreate = async () => {
    ...
    if (error) {
      ...
      queryClient.invalidateQueries({ queryKey: ["admin_all_poll_votes"] });
    }
    setSaving(false);
  };
  ...
  const getVotesForPoll = (pollId: string) =>
    allVotes?.filter((v) => v.poll_id === pollId) || [];
            ...
            {options.map((opt, i) => (
              <div key={i} className="flex gap-2">
                <Input
                  ...
                  placeholder={`অপশন ${toBengali(i + 1)}`}
                />
                {options.length > 2 && (
                  ...
                  <div className="mt-3">
                    ...
                    <div className="max-h-60 overflow-auto border rounded">
                      <Table>
                        <TableHeader>
                          <TableRow>
                            <TableHead className="text-xs">ক্রম</TableHead>
                            <TableHead className="text-xs">ভোটার আইডি</TableHead>
                            <TableHead className="text-xs">নির্বাচিত অপশন</TableHead>
                          ...
                          {pollVotes.map((vote, i) => (
                            <TableRow key={vote.id}>
                              ...
                              <TableCell className="text-xs">
                                {new Date(vote.created_at).toLocaleDateString("bn-BD")}
                              </TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
