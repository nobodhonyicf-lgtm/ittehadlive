...
const AdminPrayerTimes = () => {
  ...
  const handleSave = async () => {
    ...
    for (const item of editItems) {
      await supabase
        .from("prayer_times")
        .update({ name: item.name, time_text: item.time_text, sort_order: item.sort_order })
        .eq("id", item.id);
    }
  ...
  const handleAdd = async () => {
    if (!newName.trim() || !newTime.trim()) {
      toast({ title: "ত্রুটি", description: "নাম ও সময় দিন", variant: "destructive" });
      return;
    }
    const { error } = await supabase.from("prayer_times").insert({
      name: newName.trim(),
      time_text: newTime.trim(),
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">নামাজের সময়সূচি</h1>

      <Card>
        <CardHeader>
          <CardTitle className="text-base">সময়সূচি সম্পাদনা</CardTitle>
        </CardHeader>
          ...
          {items.map((item, i) => (
            <div key={item.id} className="flex gap-2 items-center">
              <Input
                ...
                placeholder="নামাজের নাম"
                className="flex-1"
              />
              <Input
                value={item.time_text}
                onChange={(e) => handleEdit(i, "time_text", e.target.value)}
                placeholder="সময়"
                className="w-28"
              />
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">নতুন সময় যোগ</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="flex gap-2">
            <div className="flex-1">
              <Label>নাম</Label>
              <Input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="যেমন: তাহাজ্জুদ" />
            </div>
            <div className="w-28">
              <Label>সময়</Label>
