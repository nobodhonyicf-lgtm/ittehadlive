              ...
              categories?.map((c) => (
                <TableRow key={c.id}>
                  ...
                  <TableCell className="text-right">
                    {canEdit && <Button variant="ghost" size="icon" onClick={() => { setEditId(c.id); setForm({ name: c.name, slug: c.slug, parent_id: (c as any).parent_id || "" }); setOpen(true); }}><Edit size={16} /></Button>}
