...
import { Newspaper, Tag, PlayCircle, Megaphone } from "lucide-react";
...
const VideosWidget = () => {
  ...
  return (
    <Card className="border-t-4 border-t-destructive">
      <CardHeader className="pb-2">
        <CardTitle className="text-destructive text-base bg-destructive/10 rounded py-2 text-center flex items-center justify-center gap-2">
          <PlayCircle size={16} />
          ভিডিও
        </CardTitle>
      </CardHeader>
      ...
      {ads.slice(0, 2).map((ad) => (
        ...
        >
          <img
            src={ad.image_url}
            alt={ad.title}
            className="w-full rounded-lg border hover:shadow-md transition-shadow"
          />
        </a>
      ))}
    </div>
