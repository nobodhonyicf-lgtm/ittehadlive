  ...
  return (
    <div className="bg-card border border-border rounded-lg overflow-hidden">
      ...
      <div className="p-4 space-y-4">
        {/* Iftar countdown full-width */}
        <IftarCountdownWidget />

        {/* 3-column grid for Quran, Hadith, Dua */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {(Object.keys(categoryConfig) as Array<keyof typeof categoryConfig>).map((key) => {
            ...
            return (
              <div
                key={key}
                className={`bg-gradient-to-br ${config.gradient} ${config.darkGradient} rounded-xl p-4 text-white relative overflow-hidden min-h-[180px] flex flex-col transition-transform hover:scale-[1.02] shadow-lg`}
              >
